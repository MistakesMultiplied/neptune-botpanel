...
class SandboxManager {
    constructor() {
        ...
        // Track the first time a bot is detected in a problematic state so
        // we can apply a 10-second grace period before auto-restarting.
    ...
    // priorityValue defaults to 64 -> Idle. Other common values: 16384 (Below Normal), 32 (Normal), 128 (High)
    ...
    launchInSandbox(botNumber, program, args = [], isBackground = true) {
        ...
        // Map specific executables to start priority flags and CPU affinity masks
        const priorityMap = {
            'vac-bypass-loader.exe': '/low',          // Idle priority
            'steam.exe': '/low',                      // Idle priority
            'steamwebhelper.exe': '/low',             // Idle priority
            'tf_win64.exe': '/high'                  // High priority
        };
        const affinityMap = {
            'vac-bypass-loader.exe': '1',             // CPU0 only
            'steam.exe': '1',
            'steamwebhelper.exe': '1'
            // tf_win64.exe intentionally left without affinity restriction to allow full CPU usage
        };
        const priorityFlag = priorityMap[programFileName] || '';
        const affinityFlag = affinityMap[programFileName] ? `/affinity ${affinityMap[programFileName]} ` : '';
        
        // NEW: Prepare environment variable for the bot ID so the in-game DLL can read it reliably
        const envArg = `/env:BOTID=${botNumber}`;
        ...
        try {
            ...
            let command;
            if (isBackground) {
                // Properly escape quotes for Windows cmd
                // Using /min to minimize the window instead of hiding it completely
                const argsStr = args.join(' ');
                const prioritySegment = priorityFlag ? `${priorityFlag} ` : '';
                command = `"${sandboxiePath}" /box:${sandboxName} ${envArg} cmd /c start "" ${prioritySegment}${affinityFlag}/min "${program}" ${argsStr}`;
            } else {
                command = `"${sandboxiePath}" /box:${sandboxName} ${envArg} "${program}" ${args.join(' ')}`;
            }
    ...
    launchVacBypass(botNumber, account) {
        ...
        // Fix command format: Don't use cmd /c with /elevate as it may cause issues
        // Using direct invocation with proper quoting
        const vacBypassCommand = `"${state.config.sandboxiePath}" /box:${sandboxName} /elevate "${vacBypassPath}" ${account.username} ${account.password} -nobrowser`;
        ...
        return new Promise((resolve, reject) => {
            exec(vacBypassCommand, (error, stdout, stderr) => {
                ...
                this.setProcessPriority('VAC-Bypass-Loader.exe', 64); // Idle priority
                // Steam might spawn a bit later, attempt after short delay
                setTimeout(() => {
                    this.setProcessPriority('steam.exe', 64);
                    this.setProcessPriority('steamwebhelper.exe', 64);
                }, 5000);
    ...
    createBotFile(sandboxName, botNumber) {
        ...
        // Create file in sandboxed %APPDATA%/Amalgam folder - each bot will only see its own file
        const createSandboxedAmalgamFileCommand = `"${state.config.sandboxiePath}" /box:${sandboxName} cmd /c "if not exist "%APPDATA%\\Amalgam" mkdir "%APPDATA%\\Amalgam" && echo Bot ${botNumber} initialized > "%APPDATA%\\Amalgam\\${botFileName}""`;
        ...
        try {
            ...
            // Create file in sandboxed %APPDATA%/Amalgam
            exec(createSandboxedAmalgamFileCommand, (error, stdout, stderr) => {
                if (error) {
                    logger.error(`Error creating ${botFileName} in sandboxed Amalgam folder: ${error.message}`);
                    return;
                }
                logger.info(`Created ${botFileName} in the sandboxed Amalgam folder for bot ${botNumber}`);
            });
        } catch (error) {
	...
	'-forcepreload',
        ...
        const process = this.launchInSandbox(botNumber, tf2Path, launchOptions, isBackground);
        ...
        if (process) {
            ...
            // WMIC priority setting removed (now handled by start flag)
        } else {
    ...
    checkProcesses() {
        ...
        for (const [botNumber, processInfo] of this.processes.entries()) {
            ...
            if (pipeStatus && pipeStatus.includes('Disconnected') && currentStatus === BotStatus.CONNECTED) {
                ...
                if (Date.now() - this.problemStatusTimestamps.get(botNumber) >= 10000) {
                    logger.warn(`Bot ${botNumber} has been CONNECTED with a disconnected pipe for >10s - triggering auto-restart`);
                    if (this.io) {
